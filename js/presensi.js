...
async function compressImage(base64, options = {}) {
    ...
    return new Promise((resolve, reject) => {
        ...
        img.onerror = () => reject(new Error('Gagal memuat gambar')); img.src = base64;
    });
}
...
function upLoc() {
    ...
    navigator.geolocation.getCurrentPosition(p => {
        if (p.coords.accuracy > 150) { showToast("Sinyal Lemah", `Akurasi GPS rendah (${p.coords.accuracy.toFixed(0)}m). Pastikan di luar ruangan.`, "warning"); }
...
function saveAutoRecovery() { sessionStorage.setItem('pusda_recovery', JSON.stringify({ notes: document.getElementById('notes').value, sB64, kB64, suratB64, status: selectedStatus })); }
function loadAutoRecovery() { const saved = sessionStorage.getItem('pusda_recovery'); if (saved) { const data = JSON.parse(saved); document.getElementById('notes').value = data.notes || ""; if (data.sB64) { sB64 = data.sB64; document.getElementById('sImg').src = sB64; document.getElementById('sImg').style.display = 'block'; document.getElementById('sPh').style.display = 'none'; } if (data.kB64) { kB64 = data.kB64; document.getElementById('kImg').src = kB64; document.getElementById('kImg').style.display = 'block'; document.getElementById('kPh').style.display = 'none'; } if (data.suratB64) suratB64 = data.suratB64; if (data.status) { selectedStatus = data.status; updateStatusInfo(selectedStatus); } updateNotesCounter(); updateWorkflow(); } }
