...
window.onload = () => {
    ...
    if (hasCache) {
        // ✅ INSTAN: Langsung render data lama & sembunyikan splash
        initAppUI();
        hideSplashScreen();
        // Ambil data baru di background
        fetchBackgroundData();
    } else {
        // Belum punya cache, harus tunggu fetch pertama
        fetchInitialData();
    }
...
// ============ FETCH DATA AWAL (JIKA TIDAK ADA CACHE) ============
...
// ============ FETCH DATA BACKGROUND (SILENT UPDATE) ============
async function fetchBackgroundData() {
    try {
        ...
        if (typeof newData === 'object' && newData) {
            appData = newData;
            saveToCache();
            // Re-render UI silently tanpa ganggu user
            initAppUI(); 
        }
    } catch(e) {
...
function renderMainDashboard() {
    ...
    // ✅ Panggil createIcons hanya SEKALI di akhir agar performa HP tidak drop
    lucide.createIcons();
}
