...
function parseGeoData(rawData) {
    ...
    if (typeof rawData === 'string') {
        ...
        try {
            ...
            console.warn("⚠️ Parse Koordinat_Tugas gagal:", e.message); 
        }
    }
    return [];
}
...
async function loadDashboard(forceRefresh = false) {
    const cached = sessionStorage.getItem('dashData'),
        cacheTime = sessionStorage.getItem('dashDataTime'),
        now = Date.now();
...
function updateQRRealtime() {
    const n = document.getElementById('p-nama').value.trim(),
        h = document.getElementById('p-nohp').value.trim();
    if (n || h) {
...
function renderPersonelList(list, tableId, gridId, mobileGridId, type) {
    const tE = document.getElementById(tableId),
        gE = document.getElementById(gridId),
        mE = document.getElementById(mobileGridId);
    if (!list || list.length === 0) {
        tE.innerHTML = '<tr><td colspan="9" style="text-align:center;padding:30px;opacity:.6">Tidak ada data.</td></tr>';
        if (gE) gE.innerHTML =
            '<div class="empty-state" style="grid-column:1/-1;text-align:center;padding:40px 20px;opacity:.5"><i data-lucide="users" size="40"></i><p>Belum ada data</p></div>';
    ...
    tE.innerHTML = list.map(p => {
        const id = sanitizeHTML(p.id || p.ID),
            qr = p.urlQR || p.link_qr || getQRUrl(p.nama || p.Nama, p.nohp || p.NoHP),
            foto = p.urlFoto || p.link_foto_profile || p.Link_Foto_Profile || '',
            nama = sanitizeHTML(p.nama || p.Nama),
            jab = sanitizeHTML(p.jabatan || p.Jabatan),
            wil = sanitizeHTML(p.wilayah || p.Wilayah),
            st = p.status || p.Status || 'Aktif',
            sc = st === 'Aktif' ? 'badge-aktif' : 'badge-nonaktif',
            si = st === 'Aktif' ? 'check-circle' : 'x-circle';
        let gc = 0;
        try {
            const r = p.Koordinat_Tugas || p.koordinat_tugas;
            if (r) {
                const j = JSON.parse(r);
                if (Array.isArray(j)) gc = j.length;
            }
        } catch (e) {}
...
function showDetail(type, id) {
    ...
    document.getElementById('detailStatusBadge').innerHTML =
        `<span class="badge-status ${sc}"><i data-lucide="${si}" size="12"></i> ${sanitizeHTML(st)}</span>`;
    ...
    document.getElementById('detailGeo').innerHTML = gc > 0 ?
        `<span class="geo-indicator"><i data-lucide="map-pin" size="12"></i> ${gc} Titik</span>` :
        '<span style="opacity:.5">Tidak diatur</span>';
...
function editFromDetail() {
    ...
    setTimeout(() => editP(currentDetailType, currentDetailId), 300);
}

function deleteFromDetail() {
    if (!confirm("Hapus data ini?")) return;
    closeModal('detailModal');
    setTimeout(() => deleteP(currentDetailType, currentDetailId), 300);
}

// ============ LOGS ============
async function loadLogs() {
    setLoading(true, "Memuat Data Log...");
    document.getElementById('logsBody').innerHTML = Array(5).fill(
        '<tr><td class="skeleton-cell"><div class="skeleton-line"></div></td><td class="skeleton-cell"><div class="skeleton-line"></div></td><td class="skeleton-cell"><div class="skeleton-line"></div></td><td class="skeleton-cell"><div class="skeleton-line"></div></td><td class="skeleton-cell"><div class="skeleton-line"></div></td><td class="skeleton-cell"><div class="skeleton-line"></div></td><td class="skeleton-cell"><div class="skeleton-line"></div></td><td class="skeleton-cell"><div class="skeleton-line"></div></td></tr>'
    ).join('');
    const selectedDate = document.getElementById('logDateFilter').value || new Date().toISOString().split('T')[0];
    currentLogPage = 1;
    try {
        const d = await safeFetchJSON(API + `?action=getPresensiByDate&date=${selectedDate}`, { redirect: 'follow',
        ...
        document.getElementById('logsBody').innerHTML =
            '<tr><td colspan="8" style="text-align:center;color:var(--danger);padding:50px">Gagal terhubung ke server.</td></tr>';
    } finally {
        setLoading(false);
    }
}
...
function renderLogsFiltered(resetPage = false) {
    ...
    const fD = document.getElementById('logDateFilter').value,
        q = document.getElementById('logSearch').value.toLowerCase();
    ...
    if (f.length === 0) {
        tbody.innerHTML =
            `<tr><td colspan="8" style="text-align:center;opacity:.3;padding:50px">Tidak Ada Aktivitas.<br><span style="font-size:0.8rem; margin-top:10px; display:inline-block;">Coba <a href="#" onclick="document.getElementById('logSearch').value=''; renderLogsFiltered(true);" style="color:var(--sda-toska)">reset pencarian</a>.</span></td></tr>`;
        lucide.createIcons();
        return;
    }
    const endIndex = currentLogPage * APP_CONFIG.LOGS_PER_PAGE,
        paginatedData = f.slice(0, endIndex);
    tbody.innerHTML = paginatedData.map(l => {
        const ts = l.timestamp || l.Timestamp,
            id = l['ID Pegawai'] || l.id_pegawai || l.id || l.ID,
            fs = l['Foto Selfie'] || l.foto_selfie || l.Foto_Selfie || '-',
            fk = l['Foto Kerja'] || l['Foto Lokasi'] || l.foto_kerja || l.foto_lokasi || l.Foto_Kerja || '-',
            n = parseInt(l.nilai || l.Nilai) || 0,
            sc = n >= 50 ? 'score-high' : n >= 25 ? 'score-mid' : 'score-low',
            nama = sanitizeHTML(l.nama || l.Nama),
            status = sanitizeHTML(l.status || l.Status),
            wil = sanitizeHTML(l.wilayah || l.Wilayah);
    ...
    if (endIndex < f.length) tbody.innerHTML +=
        `<tr><td colspan="8" style="text-align:center; padding: 20px;"><button class="btn-premium" style="width: auto; margin: 0 auto; height: 40px; font-size: 0.8rem;" onclick="currentLogPage++; renderLogsFiltered(false);"><i data-lucide="chevron-down" size="16"></i> Muat Lebih Banyak (${f.length - endIndex} lagi)</button></td></tr>`;
    lucide.createIcons();
}

function exportLogsToCSV() {
    const fD = document.getElementById('logDateFilter').value,
        data = logsCache.filter(l => {
            const t = l.timestamp || l.Timestamp;
            return t ? getLocalDateStr(t) === fD : false;
        });
    if (data.length === 0) {
    ...
    data.forEach(l => {
        const ts = new Date(l.timestamp || l.Timestamp).toLocaleString('id-ID'),
            nama = (l.nama || l.Nama || "-").replace(/,/g, ";"),
            status = (l.status || l.Status || "-").replace(/,/g, ";"),
            nilai = l.nilai || l.Nilai || 0,
            wilayah = (l.wilayah || l.Wilayah || "-").replace(/,/g, ";"),
            ket = (l.keterangan || l.Keterangan || "-").replace(/,/g, ";");
...
function renderConfig() {
    const q = document.getElementById('configSearch').value.toLowerCase(),
        configData = masterData.config || {},
        keys = Object.keys(configData).filter(k => k.toLowerCase().includes(q));
    if (!keys.length) {
        document.getElementById('configContainer').innerHTML =
            '<p style="opacity:.6;padding:20px;text-align:center">Tidak ada pengaturan.</p>';
        return;
    }
    ...
    keys.forEach(k => {
        ...
        else if (l.includes('radius') || l.includes('jam') || l.includes('late') || l.includes('geo')) g['📍 Geo-Fencing'].push(
        k);
...
function renderGeoList() {
    ...
    c.innerHTML = currentGeoFences.map((p, i) =>
        `<div class="geo-item"><div class="geo-item-info"><div class="geo-item-name">${sanitizeHTML(p.nama || 'Lokasi ' + (i + 1))}</div><div class="geo-item-coords">Lat: ${p.lat}, Lng: ${p.lng} | ${p.radius}m</div></div><button class="btn-remove-geo" onclick="removeGeoPoint(${i})"><i data-lucide="trash-2" size="14"></i></button></div>`
    ).join('');
    lucide.createIcons();
}
...
function addGeoPoint() {
    ...
    const n = document.getElementById('geo-nama').value.trim() || 'Lokasi ' + (currentGeoFences.length + 1),
        la = parseFloat(document.getElementById('geo-lat').value),
        lo = parseFloat(document.getElementById('geo-lng').value),
        r = parseInt(document.getElementById('geo-radius').value) || 100;
    ...
    renderGeoList();
    document.getElementById('geo-nama').value = '';
...
function openPModal(type) {
    ...
    document.getElementById('p-nohp').value = '';
    document.getElementById('p-lokasi').value = '';
    currentGeoFences = [];
    renderGeoList();
...
// ============ DELETE PERSONEL (OPTIMISTIC UI) ============
async function deleteP(type, id) {
    if (!confirm("Hapus data ini?")) return;

    // ✅ 1. Hapus instan dari array masterData frontend
    const listKey = type === 'pegawai' ? 'pegawai' : 'korlap';
    const originalData = [...masterData[listKey]];
    masterData[listKey] = masterData[listKey].filter(p => String(p.id || p.ID) !== String(id));

    // ✅ 2. Update cache sessionStorage
    try {
        const light = {
            pegawai: masterData.pegawai.map(p => ({
                id: p.id,
    ...
    // ✅ 3. Render ulang tabel instan
    if (type === 'pegawai') renderPegawai();
    else renderKorlap();
    ...
    try {
        ...
        masterData[listKey] = originalData;
        if (type === 'pegawai') renderPegawai();
        else renderKorlap();
    } finally {
        setLoading(false);
...
async function savePAction() {
    ...
    const cLat = document.getElementById('geo-lat').value.trim(),
        cLng = document.getElementById('geo-lng').value.trim();
    if (cLat && cLng) addGeoPoint();

    const isEdit = !!document.getElementById('p-old-id').value;
    const payload = {
        token,
        action: isEdit ? (type === 'pegawai' ? 'editPegawai' : 'editKorlap') : (type === 'pegawai' ? 'addPegawai' :
            'addKorlap'),
    ...
    try {
        ...
        if (d.status === 'success') {
            ...
            await loadDashboard(true); // Refresh data dari server untuk sinkronisasi penuh
...
// ============ DELETE TOOL (OPTIMISTIC UI) ============
async function deleteTool(name) {
    if (!confirm("Hapus layanan ini?")) return;

    // ✅ 1. Hapus instan dari array masterData
    const originalTools = [...masterData.tools];
    masterData.tools = masterData.tools.filter(t => String(t.Nama || t.nama) !== String(name));
    ...
    try {
        ...
        showToast("Gagal: " + e.message, "error");
        masterData.tools = originalTools; // Kembalikan jika gagal
        renderTools();
    } finally {
        setLoading(false);
    }
...
async function saveTool() {
    ...
    try {
        ...
        await loadDashboard(true); // Refresh data dari server
    } catch (e) {
        showToast("Gagal: " + e.message, "error");
    } finally {
        setLoading(false);
...
// ============ TOOLS CRUD (OLD FUNCTIONS - KEEP FOR REFERENCE) ============
function updateToolPreview() {
    const n = document.getElementById('t-nama').value || 'Nama',
        i = document.getElementById('t-ikon').value || 'layers',
        c = document.getElementById('t-warna').value;
...
function openToolModal() {
    document.getElementById('t-old-name').value = '';
...
function openLogModal(mode, n, id, ts, st, ni) {
    ...
    d.innerHTML = [...(masterData.pegawai || []), ...(masterData.korlap || [])].map(p =>
        `<option value="${p.id || p.ID}">${sanitizeHTML(p.nama || p.Nama)}</option>`
    ).join('');
...
async function saveLogAction() {
    ...
    const id = document.getElementById('manualLogPegawai').value,
        pg = [...(masterData.pegawai || []), ...(masterData.korlap || [])].find(x => String(x.id || x.ID) === String(
        id));
    if (logMode === 'add') {
        p.action = 'addLog';
        p.idPegawai = id;
